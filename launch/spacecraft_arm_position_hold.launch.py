"""Launch the free-floating arm with effort-based joint position hold."""
...
def generate_launch_description():
    ...
    position_hold = Node(
        package='spacecraft_reaction_sim', executable='arm_position_hold',
        parameters=[{'use_sim_time': True}], output='screen')

    return LaunchDescription([
        IncludeLaunchDescription(PythonLaunchDescriptionSource(manual_launch)),
        TimerAction(period=10.0, actions=[position_hold]),
    ])
